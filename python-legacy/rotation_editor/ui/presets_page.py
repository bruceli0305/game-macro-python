...
class RotationPresetsPage(QWidget):
    ...
    def _on_new(self) -> None:
        from rotation_editor.core.templates import TEMPLATE_REGISTRY, create_from_template

        # 弹出模板选择对话框
        items = []
        for key, info in TEMPLATE_REGISTRY.items():
            items.append(f"{info['icon']}  {info['name']} — {info['desc']}")

        item, ok = QInputDialog.getItem(
            self,
            "新建方案 — 选择模板",
            "选择一个模板快速开始，或选择「空白方案」从零创建：",
            items,
            0,
            False,
        )
        ...
        template_key = list(TEMPLATE_REGISTRY.keys())[selected_idx]

        # 获取方案名称
        default_name = TEMPLATE_REGISTRY[template_key]["name"]
        name, ok = QInputDialog.getText(self, "方案名称", "请输入方案名称：", text=default_name)
        if not ok:
            return

        try:
            preset_obj = create_from_template(template_key, name)
            p = self._svc.create_preset(name, preset=preset_obj)
            self._notify.info(f"已从模板创建方案: {p.name}")
            self.refresh_list()
