...
class PynputKeySender:
    ...
    - 组合键："ctrl+a" / "shift+f1" / "alt+1" 等（以 + 分隔）
    """
    ...
    # 修饰键名映射
    _MODIFIER_KEYS: Dict[str, keyboard.Key] = {
    ...
    def _resolve_key(self, token: str):
        """将单个 token 解析为 (key, is_modifier) 元组。"""
        t = token.strip().lower()
        if not t:
            return None, False

        # 修饰键
        if t in self._MODIFIER_KEYS:
            return self._MODIFIER_KEYS[t], True

        # 特殊键
        if t in self._SPECIAL_KEYS:
            return self._SPECIAL_KEYS[t], False

        # F 键：f1..f24
        if t.startswith("f") and t[1:].isdigit():
            try:
                n = int(t[1:])
                return getattr(keyboard.Key, f"f{n}"), False
    ...
    def send_key(self, key: str) -> None:
        """
        发送按键。支持组合键格式如 "ctrl+a"、"shift+f1"、"alt+1"。
        """
        ...
        for part in parts:
            k, is_mod = self._resolve_key(part)
            if k is None:
                log.warning("PynputKeySender: 无法解析按键 token '%s'，原始输入='%s'", part, key)
                return
            if is_mod:
                modifiers.append(k)
            else:
                if main_key is not None:
                    log.warning("PynputKeySender: 检测到多个非修饰键，原始输入='%s'", key)
                    return
                main_key = k

        if main_key is None:
            log.warning("PynputKeySender: 未找到主键，原始输入='%s'", key)
            return
        ...
        try:
            ...
            for mod in modifiers:
                self._ctl.press(mod)
            # 按下并释放主键
            self._ctl.press(main_key)
            self._ctl.release(main_key)
            # 释放修饰键（逆序）
            for mod in reversed(modifiers):
                self._ctl.release(mod)
        except Exception:
            log.exception("PynputKeySender: 发送按键失败 '%s'", key)
    ...
    def close(self) -> None:
        ...
        if self._close is not None:
            try:
                ...
                log.warning("HidDllKeySender: CloseDevice 调用失败", exc_info=True)
            finally:
                self._close = None

    def __enter__(self) -> "HidDllKeySender":
        return self
