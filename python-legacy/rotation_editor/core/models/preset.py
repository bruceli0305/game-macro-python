...
class RotationPreset:
    ...
    def from_dict(d: Dict[str, Any]) -> "RotationPreset":
        ...
        gtracks: List[Track] = []
        for idx, item in enumerate(gtracks_raw):
            if isinstance(item, dict):
                try:
                    gtracks.append(Track.from_dict(item))
                except Exception:
                    log.warning("Failed to parse global_tracks[%d], skipping", idx, exc_info=True)

        modes_raw = as_list(d.get("modes", []))
        modes: List[Mode] = []
        for idx, item in enumerate(modes_raw):
            if isinstance(item, dict):
                try:
                    modes.append(Mode.from_dict(item))
                except Exception:
                    log.warning("Failed to parse modes[%d], skipping", idx, exc_info=True)

        conds_raw = as_list(d.get("conditions", []))
        conds: List[Condition] = []
        for idx, item in enumerate(conds_raw):
            if isinstance(item, dict):
                try:
                    conds.append(Condition.from_dict(item))
                except Exception:
                    log.warning("Failed to parse conditions[%d], skipping", idx, exc_info=True)
