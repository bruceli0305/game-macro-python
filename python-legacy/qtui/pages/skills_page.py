...
class SkillsPage(RecordCrudPage):
        ...
        try:
            ...
            for st in getattr(s, "ammo_stages", []) or []:
                try:
                    ...
                    # 绝对坐标 → 相对坐标（与主像素保持一致）
                    try:
                        rx2, ry2 = self._cap.abs_to_rel(vx2, vy2, mon2)
                    except Exception:
                        rx2, ry2 = vx2, vy2
    ...
    def _apply_form_to_current(self, *, auto_save: bool) -> bool:
        ...
        for row in getattr(self, "_ammo_stage_rows", []) or []:
            ...
            rel_x2 = clamp_int(int(sp_x.value()), 0, 10**9)
            rel_y2 = clamp_int(int(sp_y.value()), 0, 10**9)
            # 相对坐标 → 绝对坐标（与主像素保持一致）
            try:
                vx2, vy2 = self._cap.rel_to_abs(rel_x2, rel_y2, mon2)
            except Exception:
                vx2, vy2 = rel_x2, rel_y2
    ...
    def _pick_ammo_stage_pixel(self, row_index: int) -> None:
        ...
        def _on_confirm(c) -> None:
            try:
                avx = int(c.vx)
                avy = int(c.vy)
                # 绝对坐标 → 相对坐标（与主像素取色回调保持一致）
                try:
                    rx, ry = self._cap.abs_to_rel(avx, avy, c.monitor)
                except Exception:
                    rx, ry = avx, avy
            except Exception:
                rx = int(row["x"].value())
