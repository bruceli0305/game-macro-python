...
from rotation_editor.ui.cycle_editor_page import CycleEditorPage
...
class MainWindow(QMainWindow):
    ...
    def _setup_central_widget(self) -> None:
        ...
        # 循环阶段编辑器页（基于 CyclePhase 优先级调度）
        self._page_cycle_editor = CycleEditorPage(
            ctx=self._ctx,
            session=self.services.session,
            notify=self.notify,
            parent=self,
        ...
        self._page_indices["cycle_editor"] = self._stack.addWidget(self._page_cycle_editor)
    ...
    def _open_quick_start_wizard(self) -> None:
        """打开快速开始向导。"""
        from qtui.extensions.quick_start_wizard import QuickStartWizard

        def _on_complete() -> None:
            # 向导完成后刷新各页面
            self._set_pages_context(self._ctx)
            self._refresh_profiles_ui(self._ctx.profile_name)
            self.notify.info("快速开始向导已完成！")

        dlg = QuickStartWizard(
            profile_manager=self._pm,
            profile_ctx=self._ctx,
    ...
    def _set_pages_context(self, ctx: ProfileContext) -> None:
        ...
        try:
            self._page_cycle_editor.set_context(ctx)
        except Exception:
            log.exception("MainWindow._set_pages_context: set_context failed for cycle editor page")
