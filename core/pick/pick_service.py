...
class PickService:
    """
    Step 3-3-2 change:
    - INFO/STATUS/ERROR no longer go through EventBus
    - they go through callbacks (typically UiNotify.*)
    - pick flow events remain on EventBus
    """
    ...
    def __init__(
        ...
        *,
        ...
        ui_call_soon: Optional[Callable[[Callable[[], None]], None]] = None,
        notify_info: Optional[Callable[[str], None]] = None,
        notify_status: Optional[Callable[[str], None]] = None,
        ...
        self._ui_call_soon = ui_call_soon or (lambda fn: fn())

        self._notify_info = notify_info or (lambda _msg: None)
        self._notify_status = notify_status or (lambda _msg: None)
        self._notify_error = notify_error or (lambda _msg, _detail="": None)
    ...
    # ---------- UI-thread event posting ----------
    def _post(self, et: EventType, payload) -> None:
        try:
            self._ui_call_soon(lambda: self._bus.post_payload(et, payload))
        except Exception:
            pass

    # ---------- UI notify (thread-safe if you pass UiNotify methods) ----------
    def _info(self, msg: str) -> None:
        try:
            self._notify_info((msg or "").strip())
        except Exception:
            pass

    def _status(self, msg: str) -> None:
        try:
            self._notify_status((msg or "").strip())
        except Exception:
            pass

    def _error(self, msg: str, detail: str = "") -> None:
        try:
            self._notify_error((msg or "").strip(), (detail or "").strip())
        except Exception:
            pass
    ...
    def cancel(self) -> None:
        ...
        if sess is not None:
            self._post(EventType.PICK_CANCELED, PickCanceledPayload(context=sess.context))
    ...
    def start(self, context: PickContextRef) -> None:
        ...
        self._status(f"取色模式：移动鼠标预览，按 {sess.cfg.confirm_hotkey} 确认，Esc 取消")
    ...
    def stop(self, *, reason: str) -> None:
        ...
        if sess is not None:
            self._post(EventType.PICK_MODE_EXITED, PickModeExitedPayload(context=sess.context, reason=reason))

    # ---------- bus handlers ----------
    def _on_pick_request(self, ev: Event) -> None:
        p = ev.payload
        if not isinstance(p, PickRequestPayload):
            self._error("PICK_REQUEST payload 类型错误")
            return
        if not p.context.id or not p.context.type:
            self._error("PICK_REQUEST 缺少有效 context")
            return
    ...
    def _confirm_at_current_mouse(self, sess: _SessionSnapshot) -> None:
        ...
        try:
            ...
            self._post(EventType.PICK_CONFIRMED, payload)
        ...
        try:
            while True:
                ...
                try:
                    ...
                    self._post(EventType.PICK_PREVIEW, payload)

                except Exception as e:
                    if (now - self._last_err_t) * 1000.0 >= float(sess.cfg.error_throttle_ms):
                        self._last_err_t = now
                        self._status(f"取色预览异常: {e}")
